...
export const seedDatabase = database => {
  return new Promise(async (resolve, reject) => {
    ...
    Promise.all([
      ...
      .then(() =>
        console.log(
          'Successfully dropped old collections from previous session.',
        ),
      )
      .catch(() =>
        console.log(
    ...
    async function createStaff(staff) {
      ...
      Promise.all([
        staff.updateOne(
          { _id: vito._id },
          { $push: { subordinates: { $each: [jane._id, lisa._id] } } },
        ),
      ...
      staff.updateOne(
        { _id: vito._id },
        { $push: { subordinates: { $each: [anna._id, ashley._id] } } },
      );
      ...
      staff.updateOne(
        { _id: anna._id },
        { $push: { subordinates: { $each: [jim._id, harpal._id] } } },
      );
      ...
      Promise.all([
        staff.updateOne(
          { _id: ashley._id },
          { $push: { subordinates: { $each: [nancy._id, kendrick._id] } } },
        ),
        staff.updateOne(
          { _id: kendrick._id },
          { $push: { subordinates: christine._id } },
        ),
        staff.updateOne(
          { _id: christine._id },
          { $set: { superior: kendrick._id } },
        ),
      ]);
      ...
      staff.updateOne(
        ...
        {
          $push: {
            subordinates: { $each: [george._id, eric._id, alexander._id] },
          },
        },
      );
    ...
    async function createBooks(authors, books) {
      ...
      const cookForHumans = await books.findOne({
        title: 'How To Cook For Humans',
      });
      const cookFortyHumans = await books.findOne({
        title: 'How To Cook Forty Humans',
      });
      const cookForFortyHumans = await books.findOne({
        title: 'How To Cook For Forty Humans',
      });
      const eatingPeopleIsWrong = await books.findOne({
        title: 'Eating People Is Wrong',
      });
      const caffeine = await books.findOne({
        title: 'Caffeine Killed My Family',
      });
      const howToBeCool = await books.findOne({ title: 'How To Be Cool' });
      const everythingIllegal = await books.findOne({
        title: 'Everything I Want To Do Is Illegal',
      });
      const wifeWidow = await books.findOne({
        title: 'Teach Your Wife To Be A Widow',
      });
      const goodParts = await books.findOne({
        title: 'JavaScript The Good Parts',
      });
      const tractors = await books.findOne({ title: 'Love, Sex and Tractors' });
      const avoidHugeShips = await books.findOne({
        title: 'How To Avoid Huge Ships',
      });
      const artOfKnitting = await books.findOne({
        title: 'The Manly Art Of Knitting',
      });
      const practicalPyromaniac = await books.findOne({
        title: 'The Practical Pyromaniac',
      });
      const flagsPerCountry = await books.findOne({
        title: 'Flags Per Country',
      });
      const buildingAPIs = await books.findOne({
        title: 'Building APIs With Node.js',
      });
      const nodeWebDev = await books.findOne({
        title: 'Node.JS Web Development',
      });
      const nodeDesignPatterns = await books.findOne({
        title: 'Node.js Desgin Patterns',
      });
      const nodeRightWay = await books.findOne({
        title: 'Node.js The Right Way',
      });
      const fundamentalsNode = await books.findOne({
        title: 'Fundamentals Of Node.js',
      });
      const graphqlRelay = await books.findOne({ title: 'GraphQL and Relay' });
      const startReactNative = await books.findOne({
        title: 'Getting Started With React Native',
      });
      const learningReactNative = await books.findOne({
        title: 'Learning React Native',
      });
      ...
      const functionalProgramming = await books.findOne({
        title: 'Functional Programming',
      });
      const ecmaScript6 = await books.findOne({
        title: 'Introduction To EcmaScript 6',
      });
      ...
      const fullStackJavaScript = await books.findOne({
        title: 'Full Stack JavaScript',
      });
