...
(async () => {
  ...
  /*
   * CONNECT TO DATABASE
   * On Docker, the connection string is 'mongodb://mongo:27017' ('mongo' refers to the container's name inside
   * docker-compose.*.yml) When connected to a local (non-Docker) instance of MongoDB it is 'mongodb://localhost:27017'
   */
  const mongodb = await MongoClient.connect(`${mongo.host}:${mongo.port}`, {
  ...
  httpServer.listen({ port: server.port }, () =>
    console.log(
      `🚀 Server ready at http://${server.host}:${server.port}${server.path}`,
    ),
  );
})();
